...
export function AlphaAIEditor({
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`${API}/api/v1/alpha/ai/me`, { headers: authHeaders() });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    botnameTimer.current = setTimeout(async () => {
      try {
        const res = await fetch(`${API}/api/v1/alpha/ai/check-botname?name=${config.botname}`, { headers: authHeaders() });
        const data = await res.json();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = `${API}/api/v1/alpha/ai${isExisting ? '' : ''}`;
      const body = { ...config };
  ...
  const loadPromptPreview = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/v1/alpha/ai/me/prompt`, { headers: authHeaders() });
      const data = await res.json();
  ...
  const handleRemoveTraining = async (i: number) => {
    await fetch(`${API}/api/v1/alpha/ai/training/${i}`, { method: 'DELETE', headers: authHeaders() });
  ...
  const handleRemoveKnowledge = async (i: number) => {
    await fetch(`${API}/api/v1/alpha/ai/knowledge/${i}`, { method: 'DELETE', headers: authHeaders() });
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <Field label="URL do Avatar">
                  <Input value={config.avatarUrl} onChange={set('avatarUrl')} placeholder="https://..."/>
                </Field>
                <Field label="URL do Banner">
                  <Input value={config.bannerUrl} onChange={set('bannerUrl')} placeholder="https://..."/>
                </Field>
              </div>
          ...
          {activeTab === 'behavior' && (
            <>
              ...
              <Field label="Prompt de personalidade" hint="Instruções adicionais de personalidade em linguagem natural. Complementa os campos acima.">
                <Textarea value={config.personalityPrompt} onChange={set('personalityPrompt')} rows={5} maxLength={2000}
                  placeholder={`Exemplos:\n- Quando o utilizador estiver triste, começa sempre por reconhecer o que ele sente antes de sugerir soluções.\n- Nunca usas pontos de exclamação — preferes dois pontos.\n- Quando não souberes algo, dizes "Não tenho dados sobre isso, mas..." e sugeres onde procurar.`}/>
              </Field>
              <Field label="Prompt de conhecimento" hint="Informação extra que a IA deve saber — contexto do teu projecto, universo fictício, terminologia específica.">
                <Textarea value={config.knowledgePrompt} onChange={set('knowledgePrompt')} rows={4} maxLength={2000}
                  placeholder="Ex: Este utilizador é desenvolvedor web com 5 anos de experiência em React e NestJS. O seu projecto chama-se Alpha Network..."/>
              </Field>
              <Field label="System prompt avançado" hint="Override total do prompt base. Usa só se souberes o que estás a fazer. Podes usar {{user}} para o nome do utilizador.">
                <Textarea value={config.customSystemPrompt} onChange={set('customSystemPrompt')} rows={6} maxLength={4000}
                  placeholder="Sistema completo de instruções. Este campo sobrepõe-se a todos os outros se preenchido..."/>
              </Field>
            </>
          )}
          ...
          {activeTab === 'messages' && (
            <>
              ...
              <Field label="Mensagem inicial" hint="O que a IA diz quando a conversa começa. Deixa vazio para sem mensagem automática.">
              ...
              <Field label="Mensagem de regresso" hint="Quando o utilizador volta após algum tempo de inactividade.">
                <Textarea value={config.wakeupMessage} onChange={set('wakeupMessage')} rows={3} maxLength={500}
                  placeholder="Bem-vindo de volta! Estava a pensar em ti. O que aconteceu?"/>
              </Field>
              <Field label="Mensagem de inactividade" hint="Quando a IA fica sem resposta durante muito tempo.">
                <Textarea value={config.sleepMessage} onChange={set('sleepMessage')} rows={2} maxLength={300}
                  placeholder="Parece que te perdeste... Estou aqui se precisares."/>
              </Field>
              <Field label="Mensagem de erro" hint="Quando algo corre mal com a IA.">
                <Textarea value={config.errorMessage} onChange={set('errorMessage')} rows={2} maxLength={300}
                  placeholder="Ops, algo correu mal da minha parte. Tenta outra vez?"/>
              </Field>
            </>
          )}
          ...
          {activeTab === 'training' && (
            <>
              <SectionTitle icon="🎯" title="Exemplos de Treino" desc="Mostra à IA como deve responder com exemplos concretos de conversas. Máx. 50 exemplos."/>
              {!isExisting && (
                <div style={{ padding: 14, borderRadius: 12, background: '#f97316' + '12', border: '1px solid #f97316' + '30', fontSize: 12, color: '#f97316' }}>
              ...
              <div style={{ padding: 16, borderRadius: 14, border: '1px solid var(--border)', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Adicionar exemplo</div>
                ...
                <Field label={`Resposta de ${config.name || 'Alpha'}`}>
              ...
              {/* Existing examples */}
              {config.trainingExamples.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {config.trainingExamples.length === 0 && isExisting && (
                <p style={{ fontSize: 13, color: 'var(--text-tertiary)', textAlign: 'center', padding: '20px 0' }}>
                  Nenhum exemplo ainda. Adiciona o primeiro acima!
                </p>
              )}
            </>
          )}
          ...
          {activeTab === 'knowledge' && (
            <>
              <SectionTitle icon="📚" title="Base de Conhecimento" desc="Informação específica que a tua IA deve dominar. Máx. 30 entradas."/>
              {!isExisting && (
                <div style={{ padding: 14, borderRadius: 12, background: '#f97316' + '12', border: '1px solid #f97316' + '30', fontSize: 12, color: '#f97316' }}>
              ...
              <div style={{ padding: 16, borderRadius: 14, border: '1px solid var(--border)', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Adicionar entrada</div>
                <Field label="Título">
                  <Input value={newKnowledge.title} onChange={v => setNewKnowledge(p => ({ ...p, title: v }))} placeholder="Ex: Personagens do meu universo"/>
                </Field>
                <Field label="Conteúdo">
                  <Textarea value={newKnowledge.content} onChange={v => setNewKnowledge(p => ({ ...p, content: v }))} rows={4} placeholder="Conteúdo detalhado que a IA deve saber sobre este tópico..."/>
                </Field>
          ...
          {activeTab === 'preview' && (
            <>
              <SectionTitle icon="👁️" title="Preview do System Prompt" desc="O prompt gerado que é enviado ao modelo. Aprox. tokens estimados."/>
              {promptPreview ? (
                <pre style={{
                  padding: 16, borderRadius: 14, border: '1px solid var(--border)',
                  background: 'var(--card-bg)', fontSize: 11.5, lineHeight: 1.7,
                  color: 'var(--text-secondary)', whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                  fontFamily: "'JetBrains Mono', 'Fira Code', monospace", maxHeight: 500, overflowY: 'auto',
                }}>
                  {promptPreview}
                </pre>
              ) : (
