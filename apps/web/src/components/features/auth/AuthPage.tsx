...
export default function AuthPage({ initialMode = 'login' }: Props) {
  ...
  // ── refs para animação do conteúdo do painel ──
  const pcLoginRef    = useRef<HTMLDivElement>(null);
  const pcRegisterRef = useRef<HTMLDivElement>(null);
  const busyRef       = useRef(false);

  // Sincronizar quando o prop muda (ex: navegação directa para /auth/register)
  useEffect(() => {
    setMode(initialMode);
  }, [initialMode]);

  // Inicializar estilos dos refs de acordo com o modo inicial —
  // sem isto, ao montar em /auth/register o painel fica no estado visual errado
  useEffect(() => {
    ...
    if (initialMode === 'register') {
      // Painel já está à esquerda via CSS (.card.isRegister .panel)
      // Esconder conteúdo de login, mostrar conteúdo de register
      pcL.style.transition = 'none';
      pcL.style.opacity    = '0';
      pcL.style.transform  = 'translateX(-22px)';
      pcL.style.pointerEvents = 'none';

      pcR.style.transition  = 'none';
      pcR.style.opacity     = '1';
      pcR.style.transform   = 'translateX(0)';
      pcR.style.pointerEvents = 'all';
    } else {
      // Estado padrão — login visível, register oculto
      pcL.style.opacity     = '1';
      pcL.style.transform   = 'translateX(0)';
      pcL.style.pointerEvents = 'all';

      pcR.style.opacity     = '0';
      pcR.style.transform   = 'translateX(20px)';
      pcR.style.pointerEvents = 'none';
    }
  // Apenas no mount — depende só do initialMode
  ...
  function goLogin() {
    ...
    pcR.style.opacity    = '0';
    pcR.style.transform  = 'translateX(22px)';
    pcR.style.pointerEvents = 'none';
  ...
  async function handleRegister(e: FormEvent) {
    e.preventDefault();
    setError('');
    // username = preenchido pelo campo dedicado ou derivado do nome
  ...
  useEffect(() => {
    const containerEl = document.getElementById('auth-particles');
    if (!containerEl) return;

    // Recebe o elemento como parâmetro tipado — evita o erro TS18047
    function mkParticle(target: HTMLElement) {
      ...
      el.style.cssText = `
        ...
        left:${x}%;bottom:-5px;
        --dx:${dx};
        animation:rise ${dur}s ${delay}s linear both`;
      target.appendChild(el);
      el.addEventListener('animationend', () => el.remove());
    }

    const interval = setInterval(() => mkParticle(containerEl), 380);
    for (let i = 0; i < 10; i++) mkParticle(containerEl);
    return () => clearInterval(interval);
  }, []);

  return (
    <>
      {/* Partículas — definição da keyframe no CSS Module não funciona como global,
          por isso injectamos aqui como style tag */}
      <style>{`
        @keyframes rise {
          0%   { transform: translateY(0) translateX(0); opacity: 0; }
          8%   { opacity: 0.55; }
          92%  { opacity: 0.15; }
          100% { transform: translateY(-140px) translateX(var(--dx)); opacity: 0; }
        }
      `}</style>
      ...
      <div className={`${s.card} ${isRegister ? s.isRegister : ''}`}>
        <div className={s.forms}>
          ...
          <div className={`${s.formSide} ${s.formLogin}`}>
            <div>
              <p className={s.fieldLabel} style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 7 }}>
                Bem-vindo de volta
              </p>
              <h1 style={{ fontFamily: 'Cinzel, serif', fontSize: 26, fontWeight: 700, color: '#FEFCFF', marginBottom: 4, lineHeight: 1.15 }}>
                Inicia sessão
              </h1>
              <p style={{ fontSize: 12, color: '#605A78', fontWeight: 300, marginBottom: 28 }}>
                A tua identidade digital aguarda
              </p>
              ...
              <form onSubmit={handleLogin} noValidate>
                <div className={s.field}>
                  ...
                  <div className={s.fieldWrap}>
                    ...
                    />
                    <svg className={s.fieldIcon} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <div className={s.rowSmall}>
                  <label className={s.checkLabel}>
                    <div className={s.checkBox}>
                      {remember && (
                        <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="3.5">
                ...
                <button type="submit" className={s.btnMain} disabled={loading && mode === 'login'}>
                  ...
                  {loading && mode === 'login'
                    ? <><span className={s.btnSpinner} />A entrar…</>
                    : 'Entrar na Alpha Network'
                  }
                </button>
              </form>

              <div className={s.socialSep}>
          ...
          <div className={`${s.formSide} ${s.formRegister}`}>
            <div>
              ...
              <p style={{ fontSize: 12, color: '#605A78', fontWeight: 300, marginBottom: 20 }}>
              ...
              <form onSubmit={handleRegister} noValidate>
                ...
                <button type="submit" className={s.btnMain} disabled={loading && mode === 'register'} style={{ marginTop: 4 }}>
                  ...
                  {loading && mode === 'register'
                    ? <><span className={s.btnSpinner} />A criar conta…</>
          ...
          <div className={s.panel}>
            <div className={s.panelInner}>
              ...
              {/* Conteúdo login (painel à direita) */}
              <div className={s.panelContent} ref={pcLoginRef}>
                ...
                <div className={s.modeDots}>
                  <div className={s.modeDot} style={{ background: '#E040FB' }} title="Lazer" />
                  <div className={s.modeDot} style={{ background: '#FF7043' }} title="Criador" />
                  <div className={s.modeDot} style={{ background: '#00BCD4' }} title="Developer" />
                  <div className={s.modeDot} style={{ background: '#FFD700' }} title="Comunidade" />
                  <div className={s.modeDot} style={{ background: '#76FF03' }} title="Bots" />
                </div>
              </div>

              {/* Conteúdo registo (painel à esquerda) */}
              <div className={s.panelContentRegister} ref={pcRegisterRef}>
                ...
                <div className={s.modeDots}>
                  <div className={s.modeDot} style={{ background: '#E040FB' }} />
                  <div className={s.modeDot} style={{ background: '#FF7043' }} />
                  <div className={s.modeDot} style={{ background: '#00BCD4' }} />
                  <div className={s.modeDot} style={{ background: '#FFD700' }} />
                  <div className={s.modeDot} style={{ background: '#76FF03' }} />
                </div>
              </div>
            </div>
          </div>
...
function GitHubIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="#EDE9F5">
