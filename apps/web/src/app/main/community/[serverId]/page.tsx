...
const BG_DARKEST   = '#0d0e10';
const BG_DARK      = '#111214';
const BG_MID       = '#1a1b1e';
const BG_LIGHT     = '#232428';
...
interface Member { userId: string; role: string; communityRoleId: string | null; mutedUntil?: string | null; communityRole: CommunityRole | null; profile: { displayName?: string | null; username: string; avatarUrl?: string | null; bio?: string | null } | null; }
interface BotRow { id: string; isAdminBot: boolean; bot: { id: string; name: string; prefix: string }; }
interface Server { id: string; name: string; description?: string | null; imageUrl?: string | null; inviteCode: string; ownerId: string; channelCategories: ChannelCategoryRow[]; channels: Channel[]; members: Member[]; bots: BotRow[]; roles: CommunityRole[]; membersCount: number; }
...
// NOTE: Events endpoint not yet implemented in backend - kept as local-only state
...
function fmtEventDate(d: string) { return new Date(d).toLocaleDateString('pt-PT', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }); }
...
function prevMt(msgs: Msg[], i: number) { return i > 0 ? (msgs[i - 1].messageType || 'text') : ''; }

// ─── UPLOAD HELPER ───────────────────────────────────────────────────────────
// Uses POST /community/servers/:id/upload (the only upload endpoint available)
// serverId is required to scope uploads correctly
async function uploadFileToCommunity(file: File, serverId: string): Promise<string> {
...
function EmojiPickerPopover({ onSelect, onClose }: { onSelect: (e: string) => void; onClose: () => void }) {
  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{
      ...
      background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8,
      padding: 8, display: 'grid', gridTemplateColumns: 'repeat(8, 28px)', gap: 2,
      zIndex: 50, animation: 'fadeIn 0.1s ease', boxShadow: '0 8px 24px rgba(0,0,0,0.6)',
    }}>
      {EMOJIS.map(e => (
        <button key={e} onClick={() => { onSelect(e); onClose(); }}
          title={e}
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, width: 28, height: 28, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.1s' }}
...
function ServerIcon({ server, active, onClick }: { server: MyServer; active: boolean; onClick: () => void }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', marginBottom: 8 }}>
      {(active || hovered) && (
        <div style={{ position: 'absolute', left: -4, width: 4, height: active ? 40 : 20, background: ACCENT, borderRadius: '0 4px 4px 0', transition: 'height 0.15s ease' }} />
      )}
      <button type="button" onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} title={server.name}
        style={{ width: 48, height: 48, borderRadius: active ? 16 : hovered ? 16 : '50%', background: server.imageUrl ? 'transparent' : active ? ACCENT_DIM : BG_LIGHT, border: active ? `2px solid ${ACCENT}` : '2px solid transparent', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-radius 0.15s ease, background 0.15s ease, border-color 0.15s ease', flexShrink: 0, marginLeft: 4 }}
      >
...
function Tooltip({ children, text }: { children: React.ReactNode; text: string }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex' }} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      {children}
      {show && <div style={{ position: 'absolute', left: '110%', top: '50%', transform: 'translateY(-50%)', background: '#111', color: TEXT_BRIGHT, fontSize: 12, padding: '4px 8px', borderRadius: 4, whiteSpace: 'nowrap', zIndex: 999, pointerEvents: 'none', border: `1px solid ${BORDER_SUBTLE}` }}>{text}</div>}
    </div>
  );
}
...
function EditProfileModal({ user, onClose, onSave }: {
  user: { profile?: { displayName?: string | null; username: string; avatarUrl?: string | null; bio?: string | null } | null };
  onClose: () => void;
  onSave: (data: { displayName: string; bio: string; avatarUrl?: string }) => Promise<void>;
}) {
  ...
  async function handleSave() {
    setSaving(true);
    try { await onSave({ displayName, bio }); }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 520, background: BG_MID, borderRadius: 16, overflow: 'hidden', border: `1px solid ${BORDER_SUBTLE}`, boxShadow: '0 20px 60px rgba(0,0,0,0.6)', animation: 'fadeIn 0.15s ease' }}>
        <div style={{ padding: '24px 28px', borderBottom: `1px solid ${BORDER_SUBTLE}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ color: TEXT_BRIGHT, fontSize: 18, fontWeight: 700, margin: 0 }}>Editar perfil</h2>
        ...
        <div style={{ padding: 28 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
            <div style={{ position: 'relative' }}>
              <div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', background: BG_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `3px solid ${BORDER_SUBTLE}` }}>
              ...
              {/* Avatar upload not yet supported by backend */}
            </div>
            <div>
              <p style={{ color: TEXT_BRIGHT, fontWeight: 600, margin: 0, fontSize: 16 }}>{user.profile?.username}</p>
              <p style={{ color: TEXT_MUTED, fontSize: 12, marginTop: 6 }}>@{user.profile?.username}</p>
            </div>
          </div>
          {/* Display Name */}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} disabled={saving}
              style={{ background: ACCENT, color: '#000', border: 'none', borderRadius: 8, padding: '9px 24px', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
...
function UserProfileModal({ member, server, onClose, isOwn, isMod, isAdmin, onKick, onBan, onAssignRole }: {
  ...
  const bannerColor = isOwner ? '#1a1400' : accent + '22';
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }} />
      <div style={{ position: 'relative', zIndex: 10, width: 340, background: BG_MID, borderRadius: 16, overflow: 'hidden', border: `1px solid ${BORDER_SUBTLE}`, boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
        <div style={{ height: 90, background: bannerColor, position: 'relative' }} />
        <div style={{ position: 'absolute', top: 48, left: 20 }}>
          <div style={{ width: 80, height: 80, borderRadius: '50%', border: `4px solid ${BG_MID}`, overflow: 'hidden', background: BG_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {av ? <img src={av} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ color: accent, fontSize: 28, fontWeight: 700 }}>{name[0]?.toUpperCase()}</span>}
          </div>
        </div>
        <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 8 }}>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: '50%', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, color: TEXT_MUTED, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>✕</button>
        </div>
        <div style={{ padding: '16px 20px 20px', marginTop: 36 }}>
          <div style={{ marginBottom: 12 }}>
            <h2 style={{ color: TEXT_BRIGHT, fontSize: 20, fontWeight: 700, margin: 0, lineHeight: 1.2 }}>{name}</h2>
            {username && <p style={{ color: TEXT_MUTED, fontSize: 13, margin: '4px 0 0' }}>@{username}</p>}
            {isOwner && <span style={{ display: 'inline-block', marginTop: 4, fontSize: 11, background: '#F0B13222', color: '#F0B132', border: '1px solid #F0B13244', borderRadius: 4, padding: '2px 8px' }}>👑 Dono do servidor</span>}
            {member.role === 'admin' && !isOwner && <span style={{ display: 'inline-block', marginTop: 4, fontSize: 11, background: '#ED424522', color: '#ED4245', border: '1px solid #ED424544', borderRadius: 4, padding: '2px 8px' }}>Admin</span>}
            {member.communityRole && <span style={{ display: 'inline-block', marginTop: 4, marginLeft: 4, fontSize: 11, background: (member.communityRole.color || '#7C6FAD') + '22', color: member.communityRole.color || '#7C6FAD', border: `1px solid ${member.communityRole.color || '#7C6FAD'}44`, borderRadius: 4, padding: '2px 8px' }}>{member.communityRole.name}</span>}
          </div>
          ...
          {isOwn && <div style={{ background: ACCENT_DIM, border: `1px solid ${ACCENT}44`, borderRadius: 8, padding: '8px 12px', marginBottom: 12 }}><p style={{ color: ACCENT, fontSize: 12, margin: 0 }}>Este és tu</p></div>}
          {isAdmin && !isOwner && (
            <div style={{ marginBottom: 12 }}>
...
function ChannelSettingsModal({ channel, onClose, onSave, onDelete }: {
  ...
  async function handleDelete() {
    if (!confirm(`Tens a certeza que queres eliminar #${channel.name}? Esta ação é permanente.`)) return;
    setDeleting(true);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', width: '100%', height: '100%', maxWidth: 900, margin: '0 auto' }}>
        <div style={{ width: 240, background: BG_DARK, padding: '24px 12px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ color: TEXT_MUTED, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, paddingLeft: 8 }}># {channel.name} · Canais de Texto</div>
          {(['overview', 'permissions', 'invites'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ width: '100%', textAlign: 'left', background: tab === t ? BG_HOVER : 'transparent', border: 'none', color: tab === t ? TEXT_BRIGHT : TEXT_NORMAL, padding: '8px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 14, marginBottom: 2 }}>
              {t === 'overview' ? 'Visão geral' : t === 'permissions' ? 'Permissões' : 'Convites'}
            </button>
          ))}
        ...
        <div style={{ flex: 1, background: BG_MID, padding: 40, overflowY: 'auto' }}>
          <button onClick={onClose} style={{ position: 'absolute', top: 20, right: 20, background: BG_LIGHT, border: `1px solid ${BORDER_SUBTLE}`, color: TEXT_MUTED, width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          {tab === 'overview' && (
          ...
          {tab === 'permissions' && (
            <div><h2 style={{ color: TEXT_BRIGHT, fontSize: 20, fontWeight: 700, marginBottom: 16 }}>Permissões</h2><p style={{ color: TEXT_MUTED }}>Configura quem pode ver e escrever neste canal.</p></div>
          )}
          {tab === 'invites' && (
            <div><h2 style={{ color: TEXT_BRIGHT, fontSize: 20, fontWeight: 700, marginBottom: 16 }}>Convites</h2><p style={{ color: TEXT_MUTED }}>Histórico de convites gerados para este canal.</p></div>
          )}
        </div>
      </div>
    </div>
...
function ServerSettingsModal({ server, onClose, onSave, onLeave, onDelete, onCreateRole, onDeleteRole }: {
  ...
  const bannerColors = ['#0d0e10', '#FF0080', '#FF4500', '#FF8C00', '#FFD700', '#7289DA', '#00B0F4', '#00FF7F', '#FF69B4'];
  const [bannerColor, setBannerColor] = useState('#0d0e10');
  ...
  async function handleLeave() {
    if (!confirm(`Tens a certeza que queres sair de ${server.name}?`)) return;
    setLeavingServer(true);
    try { await onLeave(); }
    catch (e: any) { alert(e.message); setLeavingServer(false); }
  }

  async function handleDeleteServer() {
    const input = prompt(`Para confirmar, escreve o nome do servidor: "${server.name}"`);
    if (input !== server.name) { if (input !== null) alert('Nome incorreto.'); return; }
    try { await onDelete(); }
    catch (e: any) { alert(e.message); }
  }
  ...
  async function handleCreateRole() {
    ...
    try {
      ...
      setRoleName(''); setRoleColor('#7C6FAD'); setRoleCanMod(false); setRoleCanManageSrv(false); setRoleCanManageCh(false);
  ...
  const isOwner = server.ownerId === server.members.find(m => m.role === 'admin' && m.userId === server.ownerId)?.userId;
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', width: '100%', height: '100%' }}>
        ...
        <div style={{ width: 260, background: BG_DARK, padding: '24px 12px', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id as any)} style={{ width: '100%', textAlign: 'left', background: tab === t.id ? BG_HOVER : 'transparent', border: 'none', color: tab === t.id ? TEXT_BRIGHT : TEXT_NORMAL, padding: '8px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 14, marginBottom: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ marginTop: 'auto', borderTop: `1px solid ${BORDER_SUBTLE}`, paddingTop: 12 }}>
            ...
            <button onClick={handleDeleteServer} style={{ width: '100%', textAlign: 'left', background: 'transparent', border: 'none', color: '#ED4245', padding: '8px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', gap: 8, opacity: 0.6 }}>
              🗑 Eliminar servidor
            </button>
          </div>
        </div>
        {/* Content */}
        <div style={{ flex: 1, background: BG_MID, overflowY: 'auto', padding: 40 }}>
          ...
          {tab === 'profile' && (
            <div style={{ maxWidth: 680 }}>
              <h2 style={{ color: TEXT_BRIGHT, fontSize: 20, fontWeight: 700, marginBottom: 24 }}>Perfil do servidor</h2>
              {/* Banner preview */}
              <div style={{ position: 'relative', marginBottom: 48 }}>
                <div style={{ height: 120, background: bannerColor, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: 12 }}>Faixa do servidor</span>
                </div>
                <div style={{ position: 'absolute', bottom: -28, left: 20, display: 'flex', alignItems: 'flex-end', gap: 12 }}>
                  <div style={{ width: 72, height: 72, borderRadius: 18, background: BG_LIGHT, border: `4px solid ${BG_MID}`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                    {server.imageUrl ? <img src={server.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ color: ACCENT, fontSize: 26, fontWeight: 700 }}>{server.name[0]}</span>}
                  </div>
                  <p style={{ color: TEXT_MUTED, fontSize: 11, marginBottom: 6 }}>Ícone não editável via API</p>
                </div>
                <input type="hidden" />
              </div>
              {/* Banner colors */}
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', color: TEXT_MUTED, fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Cor da faixa</label>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {bannerColors.map(c => (
                    <button key={c} onClick={() => setBannerColor(c)} style={{ width: 60, height: 40, background: c, borderRadius: 8, border: bannerColor === c ? `3px solid ${ACCENT}` : '2px solid transparent', cursor: 'pointer' }} />
                  ))}
                </div>
              </div>
              {/* Name */}
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <textarea value={desc} onChange={e => setDesc(e.target.value)} maxLength={200}
                  style={{ width: '100%', minHeight: 80, background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: '10px 14px', color: TEXT_BRIGHT, fontSize: 14, resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
          ...
          {tab === 'roles' && (
            <div style={{ maxWidth: 680 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <button onClick={() => setShowRoleForm(p => !p)} style={{ background: ACCENT, color: '#000', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  + Novo cargo
                </button>
              </div>
              {/* New role form */}
              {showRoleForm && (
                <div style={{ background: BG_DARK, borderRadius: 10, padding: 20, marginBottom: 20, border: `1px solid ${BORDER_SUBTLE}`, animation: 'fadeIn 0.15s ease' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <input value={roleName} onChange={e => setRoleName(e.target.value)} placeholder="Ex: Moderador" maxLength={32}
                        style={{ width: '100%', background: BG_MID, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 6, padding: '8px 12px', color: TEXT_BRIGHT, fontSize: 14, boxSizing: 'border-box' }} />
                    </div>
                    <div>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    {[
                      ...
                      <label key={p.label} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 8 }}>
                        <div onClick={() => p.set(!p.value)} style={{ width: 36, height: 20, borderRadius: 10, background: p.value ? ACCENT : BG_HOVER, position: 'relative', transition: 'background 0.2s', cursor: 'pointer', border: `1px solid ${p.value ? ACCENT : BORDER_SUBTLE}` }}>
              ...
              {server.roles.map(r => (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: BG_DARK, borderRadius: 8, marginBottom: 6, border: `1px solid ${BORDER_SUBTLE}` }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button onClick={() => onDeleteRole(r.id)} title="Eliminar cargo"
                      style={{ background: 'none', border: 'none', color: '#ED4245', cursor: 'pointer', fontSize: 14, padding: '2px 4px', borderRadius: 4, opacity: 0.7 }}
          ...
          {tab === 'members' && (
            <div style={{ maxWidth: 680 }}>
              ...
              {server.members.map(m => {
                ...
                return (
                  <div key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px', background: BG_DARK, borderRadius: 8, marginBottom: 6, border: `1px solid ${BORDER_SUBTLE}` }}>
...
function CreateEventModal({ onClose, onCreate }: {
  onClose: () => void;
  onCreate: (data: { title: string; description: string; startsAt: string; endsAt: string; location: string }) => Promise<void>;
}) {
  ...
  async function handle() {
    ...
    setSaving(true);
    try { await onCreate({ title, description, startsAt, endsAt, location }); }
    catch (e: any) { alert(e.message); }
    finally { setSaving(false); }
  }

  return (
    <SimpleModal title="Criar evento" onClose={onClose}>
      <div style={{ marginBottom: 14 }}>
        ...
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Sessão de Gaming" maxLength={64} autoFocus
          style={{ width: '100%', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: '9px 14px', color: TEXT_BRIGHT, fontSize: 14, boxSizing: 'border-box' }} />
      </div>
      <div style={{ marginBottom: 14 }}>
        ...
        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Descreve o evento..." rows={2}
          style={{ width: '100%', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: '9px 14px', color: TEXT_BRIGHT, fontSize: 14, resize: 'vertical', boxSizing: 'border-box' }} />
      </div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          ...
          <input type="datetime-local" value={startsAt} onChange={e => setStartsAt(e.target.value)}
            style={{ width: '100%', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: '9px 14px', color: TEXT_BRIGHT, fontSize: 14, boxSizing: 'border-box', colorScheme: 'dark' }} />
        </div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input value={location} onChange={e => setLocation(e.target.value)} placeholder="Ex: Canal de voz / Discord / Online"
          style={{ width: '100%', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: '9px 14px', color: TEXT_BRIGHT, fontSize: 14, boxSizing: 'border-box' }} />
      </div>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <button onClick={onClose} style={{ background: 'transparent', color: TEXT_MUTED, border: 'none', padding: '8px 16px', cursor: 'pointer', borderRadius: 6, fontSize: 13 }}>Cancelar</button>
        <button onClick={handle} disabled={saving || !title.trim() || !startsAt}
          style={{ background: ACCENT, color: '#000', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: (saving || !title.trim() || !startsAt) ? 0.5 : 1 }}>
...
export default function ServerPage() {
  ...
  // Events are stored locally only (no backend endpoint yet)
  ...
  // Close emoji picker when clicking outside messages area
  useEffect(() => {
  ...
  async function handleSaveServer(data: { name: string; description: string }) {
    if (!server) return;
    // PATCH /community/servers/:id — only name+description are accepted
    await api.patch(`/community/servers/${server.id}`, { name: data.name, description: data.description });
    await refreshServer();
  }
  async function handleLeaveServer() {
    if (!server) return;
    // DELETE /community/servers/:id/members/me
    await api.delete(`/community/servers/${server.id}/members/me`);
    router.push('/main/community');
  }
  async function handleDeleteServer() {
    // No DELETE /community/servers/:id endpoint exists in the backend yet
    alert('A eliminação de servidores ainda não está disponível no servidor. Contacta o administrador da API.');
  }
  async function handleDeleteChannel(channelId: string) {
    // No DELETE /community/channels/:id endpoint in backend yet
    // Update local state only as a workaround
    alert('A eliminação de canais ainda não está disponível na API. A remover localmente apenas.');
    setServer(p => p ? { ...p, channels: p.channels.filter(c => c.id !== channelId) } : p);
    if (channel?.id === channelId) {
      const remaining = server?.channels.filter(c => c.id !== channelId) ?? [];
      setChannel(remaining[0] ?? null);
    }
    setChannelSettingsTarget(null);
  }
  async function handleSaveChannel(channelId: string, name: string) {
    // No PATCH /community/channels/:id endpoint in backend yet
  ...
  async function handleDeleteCategory(catId: string) {
    // No DELETE /community/servers/:id/categories/:catId endpoint in backend yet
    alert('A eliminação de categorias ainda não está disponível na API.');
  }
  ...
  async function handleDeleteRole(roleId: string) {
    ...
    try {
      await api.delete(`/community/servers/${server.id}/roles/${roleId}`);
      setServer(p => p ? { ...p, roles: p.roles.filter(r => r.id !== roleId) } : p);
    } catch (e: any) { alert(e.message); }
  }
  async function handlePin(msg: Msg) {
    ...
    try {
      if (msg.pinned) {
        await api.delete(`/community/channels/${channel.id}/pins/${msg.id}`);
      } else {
        await api.post(`/community/channels/${channel.id}/pins/${msg.id}`);
      }
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      // POST /community/servers/:id/upload is the only upload endpoint
      const url = await uploadFileToCommunity(file, server.id);
      ...
      setReplyTo(null);
    } catch (err: any) { alert('Falha ao enviar ficheiro: ' + err.message); }
    finally { setUploadingFile(false); if (fileRef.current) fileRef.current.value = ''; }
  }
  async function handleCreateEvent(data: { title: string; description: string; startsAt: string; endsAt: string; location: string }) {
    // No /community/servers/:id/events endpoint in backend yet - stored locally only
    const ev = {
      id: `local_${Date.now()}`,
      title: data.title,
  ...
  async function handleSaveProfile(data: { displayName: string; bio: string; avatarUrl?: string }) {
    // PATCH /users/me exists but is currently a placeholder in the backend
    // It will be fully implemented in v2 (Adolfo's task)
    try {
      await api.patch('/users/me', { displayName: data.displayName, bio: data.bio });
    } catch {
      // Endpoint returns "Em implementação" — ignore error, close modal
    }
  ...
  const upcomingEvents = events.filter(e => new Date(e.startsAt) > new Date()).sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime());
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: BG_DARKEST, overflow: 'hidden' }}>
      ...
      <div style={{ width: 72, background: BG_DARKEST, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 12, overflowY: 'auto', flexShrink: 0, borderRight: `1px solid ${BORDER_SUBTLE}` }}>
        <button onClick={() => router.push('/main')} title="Início" style={{ width: 48, height: 48, borderRadius: '50%', background: BG_LIGHT, border: '2px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8, color: ACCENT, fontSize: 20, flexShrink: 0 }}>🏠</button>
        ...
        <button onClick={() => router.push('/main/community')} title="Adicionar servidor" style={{ width: 48, height: 48, borderRadius: '50%', background: BG_LIGHT, border: '2px dashed rgba(165,230,0,0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 8, color: ACCENT, fontSize: 22, flexShrink: 0 }}>+</button>
      </div>

      {/* ── 2. CHANNEL SIDEBAR ─────────────────────────────────────────────── */}
      <div style={{ width: 240, background: BG_DARK, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        {/* Server header */}
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${BORDER_SUBTLE}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: showServerMenu ? BG_HOVER : 'transparent', userSelect: 'none' }}
          ...
          <span style={{ color: TEXT_MUTED, fontSize: 12, transition: 'transform 0.15s', transform: showServerMenu ? 'rotate(180deg)' : 'none' }}>▼</span>
        </div>

        {/* Server dropdown */}
        {showServerMenu && (
          <div style={{ background: BG_MID, margin: 8, borderRadius: 8, border: `1px solid ${BORDER_SUBTLE}`, overflow: 'hidden', animation: 'fadeIn 0.1s ease', zIndex: 50, position: 'relative' }}>
            {[
              ...
              { label: 'Criar categoria', icon: '📁', action: () => { setShowCat(true); setShowServerMenu(false); }, admin: true },
              ...
              (!item.admin || isAdmin) && (
                <button key={i} onClick={item.action} style={{ width: '100%', textAlign: 'left', background: 'transparent', border: 'none', color: (item as any).danger ? '#ED4245' : TEXT_BRIGHT, padding: '8px 12px', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', gap: 10 }}
          ...
          {channelsByCategory.uncategorized.length > 0 && (
            <div style={{ paddingTop: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px 4px 16px', marginBottom: 2 }}>
                ...
                {canManageCh && <button onClick={() => setShowCh(true)} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '0 4px' }} title="Criar canal">+</button>}
              </div>
              ...
              <div key={cat.id} style={{ marginTop: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', margin: '0 8px 2px' }}>
                  <button type="button" onClick={() => setCollapsedCats(p => ({ ...p, [cat.id]: !p[cat.id] }))}
                    ...
                    <span style={{ fontSize: 10, transition: 'transform 0.12s', transform: collapsed ? 'rotate(-90deg)' : 'rotate(0deg)', display: 'inline-block' }}>▼</span>
                    <span style={{ flex: 1 }}>{cat.name}</span>
                  </button>
                  {canManageCh && (
                    <button onClick={e => { e.stopPropagation(); setShowCh(true); setChCategoryId(cat.id); }} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 16, padding: '2px 6px' }} title="Criar canal">+</button>
                  )}
                  {isAdmin && (
                    <button onClick={() => handleDeleteCategory(cat.id)} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 12, padding: '2px 4px', opacity: 0.5 }} title="Eliminar categoria"
                      onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.opacity = '1'; (e.currentTarget as HTMLButtonElement).style.color = '#ED4245'; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.opacity = '0.5'; (e.currentTarget as HTMLButtonElement).style.color = TEXT_MUTED; }}>
                      ×
                    </button>
                  )}
                </div>
          ...
          {(upcomingEvents.length > 0 || isAdmin) && (
            <div style={{ marginTop: 16, paddingTop: 8, borderTop: `1px solid ${BORDER_SUBTLE}` }}>
              <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px 4px 16px', marginBottom: 4 }}>
                <span style={{ flex: 1, color: TEXT_MUTED, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Eventos</span>
                {isAdmin && <button onClick={() => setShowCreateEvent(true)} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 18, padding: '0 4px' }} title="Criar evento">+</button>}
              </div>
              {upcomingEvents.slice(0, 3).map(ev => (
                <div key={ev.id} style={{ margin: '2px 8px', padding: '8px 10px', background: BG_MID, borderRadius: 6, border: `1px solid ${ACCENT}22`, cursor: 'pointer' }}
                  onMouseEnter={e => e.currentTarget.style.background = BG_HOVER}
                  onMouseLeave={e => e.currentTarget.style.background = BG_MID}>
                  <p style={{ margin: 0, color: ACCENT, fontSize: 12, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📅 {ev.title}</p>
                  <p style={{ margin: '2px 0 0', color: TEXT_MUTED, fontSize: 11 }}>{fmtEventDate(ev.startsAt)}</p>
                  {ev.location && <p style={{ margin: '1px 0 0', color: TEXT_MUTED, fontSize: 11 }}>📍 {ev.location}</p>}
                </div>
              ))}
              {upcomingEvents.length === 0 && <p style={{ color: TEXT_MUTED, fontSize: 12, padding: '2px 16px' }}>Sem eventos agendados.</p>}
            </div>
          )}
        </div>

        {/* User panel */}
        <div style={{ padding: '8px', borderTop: `1px solid ${BORDER_SUBTLE}`, background: BG_DARKEST, display: 'flex', alignItems: 'center', gap: 8 }}>
          <div onClick={() => setMemberMenuUserId(user?.id ?? null)} style={{ width: 32, height: 32, borderRadius: '50%', background: BG_LIGHT, overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: `2px solid ${ACCENT}44` }}>
          ...
          <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => setMemberMenuUserId(user?.id ?? null)}>
            ...
            <p style={{ margin: 0, color: TEXT_MUTED, fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>@{user?.profile?.username}</p>
          </div>
          <Tooltip text="Editar perfil">
            <button onClick={() => setShowEditProfile(true)} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 15, padding: 4, borderRadius: 4 }}>✏️</button>
          </Tooltip>
          <Tooltip text="Configurações">
            <button onClick={() => setShowServerSettings(true)} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 15, padding: 4, borderRadius: 4 }}>⚙️</button>
          </Tooltip>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: BG_MID }}>
        {/* Topbar */}
        {channel && (
          <div style={{ height: 48, borderBottom: `1px solid ${BORDER_SUBTLE}`, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 12, flexShrink: 0, background: BG_MID, zIndex: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              {isMod && (
                <Tooltip text="Mensagens fixadas">
                  <button onClick={() => { setShowPins(p => !p); if (!showPins) api.get<Msg[]>(`/community/channels/${channel.id}/pins`).then(setPins).catch(() => {}); }}
                    style={{ background: showPins ? ACCENT_DIM : 'none', border: 'none', color: showPins ? ACCENT : TEXT_MUTED, cursor: 'pointer', fontSize: 16, padding: '4px 6px', borderRadius: 4 }}>📌</button>
                </Tooltip>
              )}
              <Tooltip text={showMembersPanel ? 'Ocultar membros' : 'Mostrar membros'}>
                <button onClick={() => setShowMembersPanel(p => !p)}
                  style={{ background: showMembersPanel ? ACCENT_DIM : 'none', border: 'none', color: showMembersPanel ? ACCENT : TEXT_MUTED, cursor: 'pointer', fontSize: 16, padding: '4px 6px', borderRadius: 4 }}>👥</button>
              </Tooltip>
              <div style={{ display: 'flex', alignItems: 'center', background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 4, padding: '2px 8px', gap: 6 }}>
                <input placeholder={`Buscar em #${channel.name}`} style={{ background: 'transparent', border: 'none', color: TEXT_BRIGHT, fontSize: 13, width: 160 }} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 0' }} onClick={() => setEmojiPickerMsgId(null)}>
          {loadingMsgs && (
            <div style={{ display: 'flex', justifyContent: 'center', padding: 48 }}>
              <div style={{ width: 28, height: 28, border: `2px solid ${ACCENT_DIM}`, borderTopColor: ACCENT, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
            </div>
          )}
          {!loadingMsgs && msgs.length === 0 && channel && (
            <div style={{ textAlign: 'center', padding: '60px 0' }}>
              ...
              <p style={{ color: TEXT_MUTED, fontSize: 14 }}>Este é o início do canal #{channel.name}. Sê o primeiro a escrever algo.</p>
            </div>
          )}
          {msgs.map((msg, i) => {
            const prev = msgs[i - 1];
            ...
            const reactBar = <ReactionsBar rx={rx} onReact={emoji => socket?.emit('reaction.toggle', { channelId: channel?.id, messageId: msg.id, emoji })} />;

            if (grouped) return (
              <div key={msg.id} className="msg-row" style={{ display: 'flex', alignItems: 'flex-start', padding: '1px 0 1px 56px', position: 'relative', animation: 'fadeIn 0.15s ease' }}>
                <div style={{ flex: 1 }}>
            ...
            return (
              <div key={msg.id} className="msg-row" style={{ display: 'flex', alignItems: 'flex-start', padding: '8px 0 2px', gap: 16, position: 'relative', animation: 'fadeIn 0.15s ease', marginTop: 8 }}>
                <div style={{ width: 40, height: 40, flexShrink: 0, borderRadius: '50%', overflow: 'hidden', background: BG_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: isBot ? '1px solid rgba(96,165,250,0.4)' : 'none' }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  {msg.replyTo && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, color: TEXT_MUTED, fontSize: 12 }}>
                      ...
                      <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 300 }}>{msg.replyTo.content}</span>
                    </div>
                  )}
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 2 }}>
                    ...
                    {msg.pinned && <span style={{ fontSize: 10, background: ACCENT_DIM, color: ACCENT, border: `1px solid ${ACCENT}44`, borderRadius: 3, padding: '1px 5px', fontWeight: 700 }}>📌 FIXADA</span>}
          ...
          {typingNames.length > 0 && (
            <p style={{ color: TEXT_MUTED, fontSize: 13, fontStyle: 'italic', padding: '4px 0 8px' }}>{typingNames.join(', ')} {typingNames.length === 1 ? 'a escrever' : 'a escrever'}…</p>
          )}
        ...
        {/* Pins panel (inline) */}
        {showPins && (
          <div style={{ borderTop: `1px solid ${BORDER_SUBTLE}`, padding: '12px 16px', maxHeight: 240, overflowY: 'auto', background: BG_DARK, flexShrink: 0, animation: 'fadeIn 0.15s ease' }}>
            ...
            {pins.length === 0 ? <p style={{ color: TEXT_MUTED, fontSize: 13 }}>Nenhuma mensagem fixada.</p> : pins.map(p => (
              <div key={p.id} style={{ padding: '8px 10px', background: BG_MID, borderRadius: 6, marginBottom: 6, border: `1px solid ${BORDER_SUBTLE}` }}>
        ...
        <div style={{ padding: '0 16px 16px', flexShrink: 0 }}>
          {(replyTo || editing) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', marginBottom: 4, background: BG_DARK, borderRadius: '8px 8px 0 0', border: `1px solid ${BORDER_SUBTLE}`, borderBottom: 'none' }}>
              <span style={{ fontSize: 12, color: editing ? ACCENT : TEXT_MUTED }}>{editing ? '✏️ A editar mensagem' : `💬 Responder a ${replyTo?.authorName}`}</span>
          ...
          {showAttachMenu && (
            <div style={{ background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 8, padding: 8, marginBottom: 4, animation: 'fadeIn 0.1s ease' }}>
              {[
                { icon: '🖼️', label: 'Enviar imagem', accept: 'image/*' },
                { icon: '📎', label: 'Enviar ficheiro', accept: '*' },
              ].map(item => (
                <button key={item.label} onClick={() => { if (fileRef.current) { fileRef.current.accept = item.accept; fileRef.current.click(); } setShowAttachMenu(false); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', background: 'none', border: 'none', color: TEXT_NORMAL, padding: '8px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}
                  onMouseEnter={e => e.currentTarget.style.background = BG_HOVER}
          ...
          <div style={{ display: 'flex', alignItems: 'center', background: BG_LIGHT, borderRadius: (replyTo || editing) ? '0 0 12px 12px' : 12, padding: '0 12px', border: `1px solid ${BORDER_SUBTLE}`, gap: 8, opacity: connected ? 1 : 0.6 }}>
            <input ref={fileRef} type="file" hidden onChange={handleFileUpload} />
            <button onClick={() => setShowAttachMenu(p => !p)} disabled={uploadingFile}
              style={{ background: 'none', border: 'none', color: uploadingFile ? ACCENT : TEXT_MUTED, cursor: 'pointer', fontSize: 22, padding: '10px 4px', lineHeight: 1, flexShrink: 0 }} title="Adicionar anexo">
              {uploadingFile ? '⏳' : '+'}
            </button>
            <input
              ref={inputRef}
            ...
            <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
              <Tooltip text="GIF"><button style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 13, fontWeight: 700, padding: '4px 6px' }}>GIF</button></Tooltip>
              <Tooltip text="Emoji"><button style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 18, padding: '4px 6px' }}>😊</button></Tooltip>
            </div>
            <div style={{ width: 1, height: 24, background: BORDER_SUBTLE }} />
            {/* Send button */}
            <button onClick={send} disabled={!text.trim() || !connected || !channel}
              style={{ background: text.trim() && connected && channel ? ACCENT : 'transparent', border: text.trim() && connected && channel ? 'none' : `1px solid ${BORDER_SUBTLE}`, color: text.trim() ? '#000' : TEXT_MUTED, borderRadius: 8, padding: '6px 14px', fontSize: 13, fontWeight: 700, cursor: text.trim() ? 'pointer' : 'default', transition: 'background 0.15s, color 0.15s', flexShrink: 0 }}>
              ➤
            </button>
          </div>
        </div>
      ...
      {/* ── 4. MEMBERS SIDEBAR ─────────────────────────────────────────────── */}
      {showMembersPanel && (
        <div style={{ width: 240, background: BG_DARK, overflowY: 'auto', flexShrink: 0, borderLeft: `1px solid ${BORDER_SUBTLE}`, animation: 'fadeIn 0.15s ease' }}>
          <div style={{ padding: '12px 12px 8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', background: BG_DARKEST, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 6, padding: '4px 10px', gap: 6 }}>
              <span style={{ color: TEXT_MUTED, fontSize: 14 }}>🔍</span>
                  ...
                  return (
                    <button key={m.userId} onClick={() => setMemberMenuUserId(m.userId)}
                      ...
                      <div style={{ position: 'relative', flexShrink: 0 }}>
                        ...
                        <div style={{ position: 'absolute', right: -1, bottom: -1, width: 10, height: 10, borderRadius: '50%', background: typing ? '#F0B232' : ACCENT, border: `2px solid ${BG_DARK}` }} />
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ margin: 0, color: accent, fontSize: 14, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n}</p>
                        {typing && <p style={{ margin: 0, color: '#F0B232', fontSize: 11, fontStyle: 'italic' }}>a escrever…</p>}
                      </div>
                    </button>
                  );
                })}
      ...
      {showInvite && (
        <SimpleModal title="Convidar para o servidor" onClose={() => setShowInvite(false)}>
          <p style={{ color: TEXT_MUTED, fontSize: 13, marginBottom: 12 }}>Partilha este código para convidar membros para <strong style={{ color: TEXT_BRIGHT }}>{server.name}</strong></p>
          <div style={{ background: BG_DARKEST, border: `1px solid ${ACCENT}44`, borderRadius: 8, padding: '12px 16px', fontFamily: 'monospace', color: ACCENT, fontSize: 13, marginBottom: 16, wordBreak: 'break-all' }}>{server.inviteCode}</div>
          <button onClick={() => { navigator.clipboard.writeText(server.inviteCode); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
            style={{ background: ACCENT, color: '#000', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {showCreateEvent && (
        <CreateEventModal onClose={() => setShowCreateEvent(false)} onCreate={handleCreateEvent} />
      )}
    </div>
  );
}
...
function ChannelRow({ ch, active, canManage, onSelect, onSettings }: { ch: Channel; active: boolean; canManage: boolean; onSelect: (ch: Channel) => void; onSettings: (ch: Channel) => void; }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div className="ch-row" style={{ width: 'calc(100% - 16px)', margin: '1px 8px', display: 'flex', alignItems: 'center', borderRadius: 4, background: active ? BG_HOVER : 'transparent', cursor: 'pointer', position: 'relative' }}
      ...
      {canManage && hovered && (
        <div className="ch-actions" style={{ display: 'flex', gap: 2, paddingRight: 6 }} onClick={e => e.stopPropagation()}>
          <button onClick={() => onSettings(ch)} title="Editar canal" style={{ background: BG_HOVER, border: 'none', color: TEXT_MUTED, cursor: 'pointer', width: 22, height: 22, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 }}>⚙</button>
        </div>
      )}
    </div>
  );
...
function MessageBody({ msg, mt }: { msg: Msg; mt: string }) {
  ...
  if (mt === 'embed' && msg.embedJson && typeof msg.embedJson === 'object') {
    ...
    return (
      <div style={{ borderLeft: `4px solid ${e.color || '#7C6FAD'}`, paddingLeft: 12, background: 'rgba(0,0,0,0.2)', borderRadius: '0 8px 8px 0', padding: '10px 12px 10px 16px', marginTop: 2 }}>
  ...
  // Handle @everyone mention highlight
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
      {rx.map(r => (
        <button key={r.emoji} onClick={() => onReact(r.emoji)}
          style={{ background: r.me ? ACCENT_DIM : 'rgba(0,0,0,0.25)', border: `1px solid ${r.me ? ACCENT : BORDER_SUBTLE}`, borderRadius: 12, padding: '2px 8px', fontSize: 13, color: r.me ? ACCENT : TEXT_BRIGHT, cursor: 'pointer', transition: 'border-color 0.1s' }}>
          {r.emoji} {r.count}
        </button>
      ))}
    </div>
...
// ─── MESSAGE ACTIONS ─────────────────────────────────────────────────────────
function MsgActions({ show, canDel, isOwn, isMod, isPinned, showEmojiPicker, onToggleEmojiPicker, onDelete, onEdit, onReply, onReact, onPin }: {
  show: boolean; canDel: boolean; isOwn: boolean; isMod: boolean; isPinned: boolean;
  ...
  return (
    <div className="msg-actions" style={{ position: 'absolute', right: 8, top: -18, background: BG_DARK, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 6, display: 'flex', gap: 1, padding: 2, opacity: 0, transition: 'opacity 0.1s', zIndex: 10 }}>
      ...
      <Tooltip text="Responder"><button onClick={onReply} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 14, padding: '3px 6px', borderRadius: 4 }}
        onMouseEnter={ev => ev.currentTarget.style.background = BG_HOVER}
        onMouseLeave={ev => ev.currentTarget.style.background = 'none'}>↩</button></Tooltip>
      {isMod && (
        <Tooltip text={isPinned ? 'Desafixar' : 'Fixar'}>
          <button onClick={onPin} style={{ background: isPinned ? ACCENT_DIM : 'none', border: 'none', color: isPinned ? ACCENT : TEXT_MUTED, cursor: 'pointer', fontSize: 14, padding: '3px 6px', borderRadius: 4 }}
            onMouseEnter={ev => { if (!isPinned) ev.currentTarget.style.background = BG_HOVER; }}
            onMouseLeave={ev => { if (!isPinned) ev.currentTarget.style.background = 'none'; }}>📌</button>
        </Tooltip>
      )}
      {isOwn && (
        <Tooltip text="Editar"><button onClick={onEdit} style={{ background: 'none', border: 'none', color: TEXT_MUTED, cursor: 'pointer', fontSize: 14, padding: '3px 6px', borderRadius: 4 }}
          onMouseEnter={ev => ev.currentTarget.style.background = BG_HOVER}
          onMouseLeave={ev => ev.currentTarget.style.background = 'none'}>✏️</button></Tooltip>
      )}
      {canDel && (
        <Tooltip text="Apagar"><button onClick={onDelete} style={{ background: 'none', border: 'none', color: '#ED4245', cursor: 'pointer', fontSize: 14, padding: '3px 6px', borderRadius: 4 }}
          onMouseEnter={ev => ev.currentTarget.style.background = 'rgba(237,66,69,0.15)'}
          onMouseLeave={ev => ev.currentTarget.style.background = 'none'}>🗑</button></Tooltip>
      )}
    </div>
  );
...
function SimpleModal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)' }} />
      <div style={{ position: 'relative', zIndex: 10, background: BG_MID, border: `1px solid ${BORDER_SUBTLE}`, borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, maxHeight: '80vh', overflowY: 'auto', animation: 'fadeIn 0.15s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
